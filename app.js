                ...
                database.ref('users').orderByChild('username').equalTo(username.toLowerCase()).once('value')
                    .then((snapshot) => {
                        ...
                        currentUser = {
                            ...
                            name: user.fullName,
                            role: user.role || ACCESS_LEVELS.USER,
                            userId: userKey,
                            isMaster: false,
                            accessFields: user.accessFields || 'dashboard',
                            loginTime: new Date().toISOString()
                        };

                        sessionStorage.setItem('currentUser', JSON.stringify(currentUser));
                        showLoginSuccess(`Welcome ${user.fullName}!`);
                    ...
                    tbody.innerHTML = usersList.map(([key, user]) => {
                        ...
                        return `
                            <tr>
                                ...
                                <td><span class="badge bg-success">${user.username || 'N/A'}</span></td>
                                <td><span class="badge bg-warning text-dark">${user.password || 'N/A'}</span></td>
                                <td><span class="badge bg-info">${roleDisplay}</span></td>
                                <td>${user.location || 'N/A'}</td>
                                <td><span class="badge bg-${statusBadge}">${user.status ? user.status.charAt(0).toUpperCase() + user.status.slice(1) : 'Pending'}</span></td>
                                <td style="max-width: 200px;">${accessFieldsBadges}</td>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select" id="editRole" required>
                                                ...
                                                <option value="stylist_only" ${user.role === 'stylist_only' ? 'selected' : ''}>Field Executive</option>
                                            </select>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select" id="editLocation">
                                                <option value="all" ${user.location === 'all' ? 'selected' : ''}>All Locations</option>
                                                <option value="AKURE" ${user.location === 'AKURE' ? 'selected' : ''}>AKURE</option>
                                                <option value="ONDA" ${user.location === 'ONDA' ? 'selected' : ''}>ONDA</option>
                                                <option value="ADO-EKITI" ${user.location === 'ADO-EKITI' ? 'selected' : ''}>ADO-EKITI</option>
                                                <option value="ILORIN" ${user.location === 'ILORIN' ? 'selected' : ''}>ILORIN</option>
                                                <option value="OSOGBO" ${user.location === 'OSOGBO' ? 'selected' : ''}>OSOGBO</option>
                                            </select>
                                        </div>
                                        <div class="col-md-6">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="editUsername" value="${user.username || ''}" required minlength="4">
                                            <div class="invalid-feedback">Username must be at least 4 characters.</div>
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label">Password *</label>
                                            <input type="password" class="form-control" id="editPassword" placeholder="password minimum 6 characters" value="${user.password || ''}" required minlength="6">
            ...
            if (customerForm) {
                customerForm.addEventListener('submit', function (e) {
                    ...
                    const requiredFields = {
                        'customerName': customerForm.querySelector('input[name="customerName"]'),
            ...
            form.querySelectorAll('input').forEach(input => {
                if (input.hasAttribute('required') && (!input.value || input.value.trim() === '')) {
                    ...
                    if (input.type === 'checkbox' && !input.checked) {
                        ...
                        hasErrors = true;
                        if (input.name === 'dataAgreement') {
                            errorMessages.push('Personal data agreement: You must agree to carry personal data');
                        } else {
                            errorMessages.push(`${input.name || input.id}: This checkbox is required`);
                        }
                    } else if (input.type !== 'checkbox') {
                        input.style.border = '2px solid #dc3545';
                        hasErrors = true;
                        errorMessages.push(`${input.name || input.id}: This field is required`);
                    }
        ...
        function proceedWithCustomerSubmission(form, formData, submitBtn) {
            const customerData = {
                ...
                customerNumber: formData.get('customerNumber') || 'Not Provided',
            ...
            tbody.innerHTML = customersList.map(([key, data]) => {
                ...
                return `
                    <tr data-customer="${JSON.stringify(data).replace(/"/g, '&quot;')}">
                        ...
                        <td>
                            <div>
                                <strong>${data.customerName || 'N/A'}</strong>
                                <br><small class="text-muted">ID: ${data.customerId || 'N/A'}</small>
                            </div>
                        </td>
                        <td>
                            <div>
        ...
        function displayUsers(userEntries) {
            ...
            tbody.innerHTML = userEntries.map(([phoneKey, user]) => `
                <tr>
                    ...
                    <td>${user.location === 'all' ? 'All Locations' : (user.location || 'N/A')}</td>
                    <td>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm">
                            <button class="btn btn-outline-primary" onclick="editUser('${phoneKey}')">
                                <i class="fas fa-edit"></i>
                            </button>
